...
const AdminDashboard: React.FC = () => {
    ...
    const getActivityIcon = (type: ActivityLogEntry['type']) => {
        switch (type) {
            case 'user_create': return <UserPlusIcon className="w-[18px] h-[18px] text-primary" />;
            case 'course_create': return <BookUpIcon className="w-[18px] h-[18px] text-success" />;
            case 'quiz_submit': return <FileCheckIcon className="w-[18px] h-[18px] text-accent" />;
            case 'user_login': return <LogInIcon className="w-[18px] h-[18px] text-muted" />;
            case 'user_delete': return <UserMinusIcon className="w-[18px] h-[18px] text-error" />;
            default: return <InfoIcon className="w-[18px] h-[18px] text-muted" />;
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-3 lg:grid-cols-6">
                <StatCard icon={<UsersIcon className="w-[22px] h-[22px] text-primary" />} title="Total Users" value={stats.users} />
                <StatCard icon={<GraduationCapIcon className="w-[22px] h-[22px] text-primary" />} title="Students" value={stats.students} />
                <StatCard icon={<PresentationIcon className="w-[22px] h-[22px] text-primary" />} title="Instructors" value={stats.mentors} />
                <StatCard icon={<ShieldIcon className="w-[22px] h-[22px] text-primary" />} title="Admins" value={stats.admins} />
                <StatCard icon={<LibraryIcon className="w-[22px] h-[22px] text-primary" />} title="Total Courses" value={stats.courses} />
                <StatCard icon={<FileQuestionIcon className="w-[22px] h-[22px] text-primary" />} title="Total Quizzes" value={stats.quizzes} />
            </div>
                        ...
                        {recentActivity.length > 0 ? (
                            <ul className="space-y-4">
                                {recentActivity.slice(0, 5).map((activity) => (
                                    <li key={activity.id} className="flex items-start gap-4 p-3 bg-background-secondary rounded-lg border border-default">
                            ...
                            <p className="text-muted text-center py-4">No system activity yet.</p>
                        )}
                    </CardContent>
                </Card>
            </div>
...
const StatCard: React.FC<{ icon: React.ReactNode; title: string; value: string | number }> = ({ icon, title, value }) => (
    <Card className="flex flex-col items-center text-center p-4">
        ...
        <dd className="text-3xl font-bold mt-2 text-text">{value}</dd>
