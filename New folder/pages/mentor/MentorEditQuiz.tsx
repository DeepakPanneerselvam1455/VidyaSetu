...
const MentorEditQuiz: React.FC = () => {
    ...
    if (isLoading) return <div className="text-center p-8 text-muted">Loading Quiz Editor...</div>;
    if (!quiz || !course) return <div className="text-center p-8 text-error">{error || "Quiz could not be loaded."}</div>;

    return (
        <div className="space-y-6">
            <div>
                <Link to={`/mentor/course/${quiz.courseId}`} className="text-sm text-primary hover:underline">← Back to Course</Link>
                <h1 className="text-4xl font-bold tracking-tight mt-1 text-main">Edit Quiz</h1>
            </div>

            <Card>
                <CardHeader>
                    <CardTitle className="text-main">Quiz Details</CardTitle>
                    <CardDescription className="text-muted">Update the general information for your quiz.</CardDescription>
                </CardHeader>
                <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label htmlFor="quizTitle" className="block text-sm font-medium mb-1 text-main">Quiz Title</label>
                        <Input id="quizTitle" value={quiz.title} onChange={e => handleQuizChange('title', e.target.value)} />
                    </div>
                    <div>
                        <label htmlFor="quizDifficulty" className="block text-sm font-medium mb-1 text-main">Difficulty</label>
                    ...
                    <div>
                        <label htmlFor="quizDuration" className="block text-sm font-medium mb-1 text-main">Duration (minutes)</label>
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-main">Questions ({quiz.questions.length})</CardTitle>
                    <CardDescription className="text-muted">Edit, add, or remove questions below.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {quiz.questions.map((q, index) => (
                        <Card key={q.id} className="bg-surface border-default">
                            <CardHeader className="flex-row items-center justify-between">
                                <CardTitle className="text-lg text-main">Question {index + 1}</CardTitle>
                                <Button variant="ghost" size="icon" onClick={() => handleDeleteQuestion(index)}><TrashIcon className="w-4 h-4 text-error" /></Button>
                            </CardHeader>
                            <CardContent className="space-y-3">
                                <div className="space-y-1">
                                    <label className="text-sm font-medium text-main">Question Text</label>
                                ...
                                {q.type === 'multiple-choice' && (
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            <label className="text-sm font-medium text-main">Options</label>
                                ...
                                {q.type === 'short-answer' && (
                                    <div className="space-y-1">
                                        <label className="text-sm font-medium text-main">Correct Answer</label>
                                        <Input value={q.correctAnswer} onChange={e => handleQuestionChange(index, 'correctAnswer', e.target.value)} />
                                    </div>
                                )}
                                <div className="space-y-1">
                                    <label className="text-sm font-medium text-main">Points</label>
