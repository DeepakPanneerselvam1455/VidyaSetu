        ...
        <div className="relative w-full h-56" onMouseLeave={() => setHoveredIndex(null)}>
            <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="w-full h-full" aria-label="Score trend chart">
                ...
                {pointCoordinates.map(({ x }, index) => (
                    <text key={`x-label-${index}`} x={x} y={SVG_HEIGHT - PADDING + 15} textAnchor="middle" className="text-xs fill-current text-muted">
                        {data[index].date}
                    </text>
                ))}
                {/* Line Path */}
                <path d={pathData} fill="none" strokeWidth="2" className="text-primary stroke-current" />
                {/* Points & Hover Area */}
                {pointCoordinates.map(({ x, y }, index) => (
                    <g key={index}>
                        <circle cx={x} cy={y} r={hoveredIndex === index ? 6 : 4} className="text-primary fill-current transition-all stroke-surface" strokeWidth={2} />
            ...
            {hoveredIndex !== null && (
                <div
                    className="absolute p-2 text-xs bg-surface border border-default text-main rounded-md shadow-lg pointer-events-none transform -translate-x-1/2 -translate-y-full transition-opacity z-10"
...
const getPerformanceStatus = (avgScore: number): { status: 'Excellent' | 'Average' | 'Poor' | 'N/A'; color: string } => {
    if (isNaN(avgScore)) return { status: 'N/A', color: 'bg-muted' };
    if (avgScore >= 80) return { status: 'Excellent', color: 'bg-green-600' }; // Keep specific colors for badges for now, or use 'bg-success' if available in global css but usually badges need specific bg
    if (avgScore >= 50) return { status: 'Average', color: 'bg-yellow-500' };
...
const StudentProgressCard: React.FC<{
    ...
    return (
        <Card className="bg-surface border-default">
            <CardHeader onClick={onToggle} className="cursor-pointer hover:bg-background transition-colors">
                <div className="flex justify-between items-center">
                    <CardTitle className="text-main">{studentData.studentName}</CardTitle>
                    <div className="flex items-center gap-4 text-sm text-muted">
                        <span>{Object.keys(studentData.courses).length} Course(s)</span>
                        <ChevronDownIcon className={cn("w-5 h-5 transition-transform text-muted", isExpanded && "rotate-180")} />
                    </div>
                </div>
            </CardHeader>
            {isExpanded && (
                <CardContent className="pt-4 border-t border-default space-y-6">
                    {Object.values(studentData.courses).map(({ course, progress, attempts, lastActivity, performance, chartData }) => (
                        <div key={course.id} className="p-4 border border-default rounded-lg bg-background">
                            <h3 className="text-lg font-semibold text-main">{course.title}</h3>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-3 text-sm">
                                <div>
                                    <p className="text-muted">Material Completion</p>
                                    <p className="font-semibold text-main">{progress}%</p>
                                </div>
                                <div>
                                    <p className="text-muted">Last Activity</p>
                                    <p className="font-semibold text-main">{lastActivity}</p>
                                </div>
                                <div>
                                    <p className="text-muted">Avg. Score</p>
                                    <p className="font-semibold text-main">{performance.avgScore}%</p>
                                </div>
                                <div>
                                    <p className="text-muted">Performance</p>
                                    <Badge className={cn("text-white", performance.color)}>{performance.status}</Badge>
                                </div>
                            </div>

                            <h4 className="text-md font-semibold mt-4 mb-2 text-main">Quiz Score Trend</h4>
                ...
                for (const attempt of mentorAttempts) {
                    ...
                    if (!analyticsData[studentId].courses[course.id]) {
                        analyticsData[studentId].courses[course.id] = {
                            ...
                            progress: 0,
                            lastActivity: 'N/A',
                            performance: { status: 'N/A', avgScore: NaN, color: 'bg-muted' },
                            chartData: []
                        };
                    }
