...
const MentorAddCourse: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto p-8 bg-surface border-2 border-default rounded-xl shadow-xl">
            <div className="flex items-center gap-3 mb-8">
                <FileTextIcon className="w-8 h-8 text-muted" />
                <h1 className="text-3xl font-bold text-main">New Course</h1>
            </div>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div className="md:col-span-2 space-y-4 pt-4 border-t border-default">
                        <h2 className="text-xl font-bold text-main">Course Materials ({materials.length})</h2>
                        {materials.length > 0 && (
                            <div className="space-y-2 rounded-md border border-default p-2 max-h-60 overflow-y-auto">
                                {materials.map(mat => (
                                    <div key={mat.tempId} className="flex items-center justify-between p-2 bg-background rounded">
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            {getMaterialIcon(mat.type)}
                                            <div className="overflow-hidden">
                                                <p className="font-semibold text-main truncate">{mat.title}</p>
                                                <p className="text-xs text-muted truncate">{mat.resource}</p>
                                            </div>
                                        </div>
                                        <Button type="button" variant="ghost" size="sm" className="text-error hover:text-error/80" onClick={() => handleRemoveMaterial(mat.tempId)}>Remove</Button>
                                    </div>
                                ))}
                            </div>
                        )}
                        <div className="p-4 border border-dashed border-default rounded-lg space-y-4">
                            <h3 className="font-semibold text-main">Add a New Material</h3>
                            ...
                            <Button type="button" variant="secondary" onClick={handleAddMaterial} className="bg-background-secondary text-main hover:bg-background/80">Add Material</Button>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-center pt-6">
                    <Button type="submit" disabled={isSubmitting} className="bg-primary hover:bg-primary/90 text-on-primary rounded-full px-10 py-3 text-lg h-auto flex items-center gap-2">
...
const getMaterialIcon = (type: MaterialType) => {
    switch (type) {
        case 'Link': return <LinkIcon className="w-5 h-5 text-primary shrink-0" />;
        case 'PDF': return <FileTextIcon className="w-5 h-5 text-primary shrink-0" />;
        case 'Youtube': return <VideoIcon className="w-5 h-5 text-primary shrink-0" />;
        default: return null;
    }
}
