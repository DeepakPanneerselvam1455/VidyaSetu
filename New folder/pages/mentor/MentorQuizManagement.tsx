    ...
    return (
        <div className="space-y-6">
            {!isTabView ? (
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold tracking-tight text-text">{course.title} Quizzes</h1>
                        <p className="text-text-muted">Manage and assign quizzes for this course.</p>
                    </div>
            ...
            {quizzes.length > 0 ? (
                        ...
                        <Card key={quiz.id} className="flex flex-col">
                            ...
                            <CardContent className="flex-grow space-y-4">
                                ...
                                {course.topics.length > 0 && (
                                    <div>
                                        <p className="text-xs font-semibold uppercase tracking-wider text-text-muted mb-2">Course Topics</p>
                ...
                <div className="text-center py-16 border-2 border-dashed border-border rounded-lg bg-background-secondary">
                    <HelpCircleIcon className="w-[62px] h-[62px] mx-auto text-primary" />
                    <p className="mt-4 text-lg font-semibold text-text">No Quizzes Yet</p>
                    <p className="text-text-muted">Click "Create New Quiz" to start building assessments.</p>
                </div>
            )}
            ...
            <div className="space-y-4">
                {successMessage ? (
                    <div className="text-center p-4 bg-green-500/10 rounded-md">
                        <p className="font-semibold text-green-600 dark:text-green-400">{successMessage}</p>
                    </div>
                ) : (
                    <>
                        <div>
                            <label className="block text-sm font-medium mb-2">Select Students</label>
                            <div className="border rounded-md max-h-60 overflow-y-auto p-2 border-border">
                                <div className="flex items-center p-2 border-b border-border">
                                    ...
                                    />
                                    <label htmlFor="select-all" className="font-medium text-text">Select All</label>
                                </div>
                                {students.map(student => (
                                    <div key={student.id} className="flex items-center p-2 rounded hover:bg-secondary">
                                        <input
                                            type="checkbox"
                                        ...
                                        />
                                        <label htmlFor={`student-${student.id}`} className="text-text">{student.name} ({student.email})</label>
                                    </div>
                                ))}
                            </div>
                        </div>
