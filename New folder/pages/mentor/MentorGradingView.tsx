        ...
        <div className="space-y-4">
            {studentSubmissions.length > 0 ? (
                ...
                <div className="text-center py-16 border-2 border-dashed border-default rounded-lg">
                    <CoffeeIcon className="w-12 h-12 mx-auto text-muted" />
                    <p className="mt-4 text-lg font-semibold text-main">All Caught Up!</p>
                    <p className="text-muted">No submissions are waiting for your review in this course.</p>
                </div>
            )}
        </div>
    );
...
const StudentGradingCard: React.FC<{
    ...
    const getPerformanceStatus = (score: number) => {
        if (score >= 80) return { label: 'Excellent', className: 'bg-success/10 text-success border-success/20' };
        if (score >= 50) return { label: 'Average', className: 'bg-warning/10 text-warning border-warning/20' };
        return { label: 'Needs Improvement', className: 'bg-error/10 text-error border-error/20' };
    };
    ...
    return (
        <Card>
            <CardHeader onClick={onToggle} className="cursor-pointer hover:bg-background-secondary transition-colors">
            ...
            {isExpanded && (
                <CardContent className="pt-4 border-t border-default space-y-3">
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    unansweredCount > 0 && !submission.attempt.gradedAt && "bg-warning/5 border-warning/30"
                                )}
                            >
                                <div
                                    className="flex justify-between items-center p-4 cursor-pointer hover:bg-background-secondary"
                                    onClick={() => setExpandedAttemptId(prev => prev === submission.attempt.id ? null : submission.attempt.id)}
                                >
                                    <div>
                                        <p className="font-semibold text-main">{submission.quizTitle}</p>
                                        <p className="text-sm text-muted">Submitted: {new Date(submission.attempt.submittedAt).toLocaleString()}</p>
                                    </div>
                                ...
                                {expandedAttemptId === submission.attempt.id && (
                                    <div className="p-4 border-t border-default bg-background-secondary/30">
                                        <AttemptGradingForm
    ...
    return (
        <div className="space-y-4">
            {quiz.questions.map((q, index) => {
                ...
                return (
                    <Card key={q.id} className={cn(isCorrect ? 'bg-success/5' : 'bg-error/5')}>
                        <CardHeader>
                            <p className="font-semibold">{index + 1}. {q.question}</p>
                        </CardHeader>
                        <CardContent className="space-y-2 text-sm">
                            <p><strong className="text-muted">Student's Answer:</strong> {studentAnswer}</p>
                            <p><strong className="text-muted">Correct Answer:</strong> {q.correctAnswer}</p>
                            {q.type === 'short-answer' && (
                            ...
                            />
                            <div className="mt-2">
                                <p className="text-xs font-semibold text-muted mb-1">Quick Feedback:</p>
            ...
            <Card>
                ...
                <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        />
                        <span className="text-muted">/ {editableAttempt.totalPoints}</span>
                    </div>
                </CardContent>
            </Card>
