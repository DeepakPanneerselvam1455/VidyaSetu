...
const MentorGenerateQuiz: React.FC = () => {
    ...
    if (courses.length === 0) {
        return (
            <div className="text-center p-8 bg-background-secondary rounded-lg">
                <h2 className="text-xl font-semibold mb-4 text-text">No Courses Found</h2>
                <p className="text-text-muted mb-6">You need to create a course before you can generate a quiz for it.</p>
        ...
        return (
            <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
                <div className="text-center space-y-2">
                    <h1 className="text-4xl font-extrabold tracking-tight text-main">Create New Quiz</h1>
                    <p className="text-muted">Choose your preferred method to build an assessment.</p>
                </div>

                <div className="grid md:grid-cols-2 gap-8">
                    {/* AI Mode Card */}
                    <Card
                        className="group relative overflow-hidden border-2 border-transparent hover:border-primary transition-all cursor-pointer bg-card shadow-xl"
                        onClick={() => setCreationMode('ai')}
                    >
                        <div className="absolute top-0 right-0 p-3">
                            <Badge className="bg-primary text-primary-foreground border-0">Recommended</Badge>
                        </div>
                        <CardHeader className="text-center pt-10">
                            ...
                            <CardTitle className="text-2xl text-main">Magic AI Generator</CardTitle>
                        ...
                        <CardContent className="space-y-3 pb-8 text-sm text-muted text-center">
                            <p>✓ Strictly follows course materials</p>
                            <p>✓ Instant pedagogical analysis</p>
                            <p>✓ Multiple choice or mixed formats</p>
                            <Button className="mt-4 w-full bg-primary hover:bg-primary/90">Start Generating</Button>
                        </CardContent>
                    </Card>
                    ...
                    >
                        <CardHeader className="text-center pt-10">
                            ...
                            <CardTitle className="text-2xl text-main">Pro Manual Builder</CardTitle>
                            <CardDescription className="text-base mt-2 text-muted">
                        ...
                        <CardContent className="space-y-3 pb-8 text-sm text-muted text-center">
                            ...
                            <Button variant="outline" className="mt-4 w-full border-primary text-primary hover:bg-primary/10">Start Building</Button>
                        </CardContent>
                    </Card>
                </div>
            </div>
    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto pb-20">
            <div className="flex justify-between items-center">
                <Button variant="ghost" size="sm" onClick={() => setCreationMode('selection')} className="text-text-muted">
                    ← Back to selection
                </Button>
                ...
                <form onSubmit={handleGenerate}>
                    <CardContent className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label htmlFor="course" className="block text-sm font-semibold mb-2 text-main">Target Course</label>
                                <Select id="course" value={selectedCourse?.id || ''} onChange={e => setSelectedCourse(courses.find(c => c.id === e.target.value) || null)}>
                                    {courses.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
                                </Select>
                            </div>
                            <div>
                                <label className="block text-sm font-semibold mb-2 text-main">Topic Refinement <span className="text-primary">*</span></label>
                        ...
                        {/* File Upload Zone */}
                        <div>
                            <label className="block text-sm font-semibold mb-2 text-main">Knowledge Source Materials</label>
                            <div
                                className="border-2 border-dashed border-default rounded-xl p-8 bg-surface hover:bg-background-secondary transition-all text-center cursor-pointer group"
                            ...
                            >
                                ...
                                <div className="w-14 h-14 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
                                    <FileUpIcon className="w-7 h-7 text-primary" />
                                </div>
                                <p className="font-bold text-main">Upload learning materials</p>
                                <p className="text-sm text-muted mt-1">Drag & drop your academic files here (PDF, Word, TXT, Excel, etc.)</p>
                            </div>

                            {uploadedFiles.length > 0 && (
                                <div className="mt-4 space-y-2">
                                    {uploadedFiles.map((f, i) => (
                                        <div key={i} className="flex items-center justify-between p-3 bg-surface rounded-lg border border-default">
                                            <div className="flex items-center gap-3">
                                                <div className="p-2 bg-primary/10 rounded text-primary">
                                                    {f.status === 'processing' ? <LoaderIcon className="w-4 h-4 animate-spin" /> : <FileIcon className="w-4 h-4" />}
                                                </div>
                                                <div>
                                                    <p className="text-sm font-medium text-main">{f.file.name}</p>
                        ...
                        {suggestedTopics.length > 0 && (
                            <div className="space-y-2">
                                <p className="text-xs font-bold uppercase tracking-widest text-muted">Quick Suggestions:</p>
                                <div className="flex flex-wrap gap-2">
                                    {suggestedTopics.map((sTopic, i) => (
                                        <Badge key={i} variant="secondary" className="cursor-pointer hover:bg-primary/20 transition-colors py-1.5 px-3 bg-primary/10 text-primary" onClick={() => setTopic(sTopic)}>
                                            {sTopic}
                                        </Badge>
                                    ))}
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-default">
                            <div>
                                <label htmlFor="difficulty" className="block text-sm font-semibold mb-2 text-main">Target Difficulty</label>
                            ...
                            <div>
                                <label htmlFor="questionType" className="block text-sm font-semibold mb-2 text-main">Structure</label>
                            ...
                            <div>
                                <label htmlFor="numQuestions" className="block text-sm font-semibold mb-2 text-main">Item Count</label>
                    ...
                    <CardFooter className="flex-col items-stretch pb-8">
                        ...
                        <Button type="submit" disabled={isLoading || !topic.trim()} className="w-full h-12 text-lg shadow-lg shadow-primary/20 bg-primary hover:bg-primary/90">
                            {isLoading ? (
            ...
            {generatedQuestions.length > 0 && (
                <div className="space-y-6 animate-in slide-in-from-bottom-5 duration-500">
                    <Card className="border-primary/20 shadow-xl bg-surface">
                        <CardHeader className="bg-background-secondary border-b border-default">
                            <div className="flex justify-between items-center">
                                <div>
                                    <CardTitle className="text-xl text-main">Preview & Edit Draft</CardTitle>
                                    <CardDescription className="text-muted">Review items before publishing. All content is derived from your source files.</CardDescription>
                                </div>
                                <div className="flex flex-col items-end gap-1">
                                    <Badge variant="outline" className="text-primary border-primary/20 bg-primary/5">Source: File Based</Badge>
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                                <div>
                                    <label htmlFor="quizTitle" className="block text-xs font-bold uppercase tracking-widest text-muted mb-1.5">Quiz Title</label>
                                    <Input id="quizTitle" value={quizTitle} onChange={e => setQuizTitle(e.target.value)} required className="font-semibold" />
                                </div>
                                <div>
                                    <label htmlFor="quiz-duration" className="block text-xs font-bold uppercase tracking-widest text-muted mb-1.5">Time Limit (min)</label>
                        ...
                        <CardContent className="space-y-6 pt-6">
                            {generatedQuestions.map((q, index) => (
                                <div key={q.id} className="p-5 border border-default rounded-xl space-y-4 bg-surface transition-shadow hover:shadow-sm">
                                    <div className="flex justify-between items-start gap-4">
                                        <div className="flex items-center gap-3">
                                            <span className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center font-bold text-sm text-primary">
                                                {index + 1}
                                            </span>
                                            {q.bloomsTaxonomy && (
                                    ...
                                    <div className="space-y-4 pl-1">
                                        <div>
                                            <label className="text-xs font-bold text-muted uppercase mb-1 block">Question Text</label>
                                                ...
                                                {q.options.map((opt, i) => (
                                                    <div key={i} className="flex gap-2">
                                                        <div className="flex-1">
                                                            <label className="text-[10px] font-bold text-muted uppercase mb-0.5 block">Option {i + 1}</label>
                                                            <Input
                                                                value={opt}
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-default">
                                            <div>
                                                <label className="text-[10px] font-bold text-muted uppercase mb-1 block">Correct Key</label>
                                                <Input value={q.correctAnswer} onChange={e => handleQuestionChange(index, 'correctAnswer', e.target.value)} className="bg-background-secondary border-dashed" />
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-bold text-muted uppercase mb-1 block">Points Allocation</label>
                                                <Input type="number" value={q.points} onChange={e => handleQuestionChange(index, 'points', e.target.value)} />
                                            </div>
                                        </div>

                                        {feedback[q.id] && (
                                            <div className="p-4 bg-primary/5 border border-primary/20 rounded-xl text-sm italic text-primary flex gap-3 animate-in fade-in duration-300">
                        ...
                        <CardFooter className="flex-col items-stretch gap-4 pt-4 border-t border-default">
                            <ErrorAlert message={saveError} />
                            <div className="flex flex-col sm:flex-row gap-4">
                                <Button onClick={handleSaveQuiz} disabled={isSaving} variant="outline" className="flex-1 h-11 border-default text-main">
                                    {isSaving ? 'Processing...' : 'Save Draft'}
                                </Button>
                                <Button onClick={handleSaveAndAssign} disabled={isSaving} className="flex-1 h-11 bg-primary hover:bg-primary/90 text-on-primary shadow-lg shadow-primary/10">
