...
const MentorCourseDetail: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            <div>
                <Link to="/mentor/courses" className="text-sm text-primary hover:underline">← Back to Courses</Link>
                <h1 className="text-4xl font-bold tracking-tight mt-1 text-main">{course.title}</h1>
                <p className="text-lg text-muted mt-1">{course.description}</p>
            </div>
...
const CourseMaterialsView: React.FC<{ course: Course; onUpdate: () => void }> = ({ course, onUpdate }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col lg:flex-row gap-4 p-4 bg-surface border rounded-xl border-dashed border-default">
                <div className="flex-1 space-y-1">
                    <p className="text-xs font-bold uppercase tracking-wider text-muted mb-2">Quick Add Video Material</p>
                ...
                <div className="flex items-end gap-2 shrink-0">
                    ...
                    <Button variant="outline" onClick={() => setIsAddModalOpen(true)} className="w-full lg:w-auto text-main border-default hover:bg-surface">
            ...
            {course.materials.length > 0 ? (
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {course.materials.map((material, index) => (
                        <Card
                            ...
                            className="cursor-grab active:cursor-grabbing transition-shadow bg-surface border-default"
                        >
                            <CardHeader className="flex flex-row items-start gap-4">
                                <GripVerticalIcon className="w-5 h-5 text-muted shrink-0 mt-1 cursor-grab" />
                                {getMaterialIcon(material.type)}
                                <div className="flex-1 overflow-hidden">
                                    <CardTitle className="text-lg text-main">{material.title}</CardTitle>
                                    <CardDescription className="truncate text-muted" title={material.url}>{material.url}</CardDescription>
                                </div>
                            </CardHeader>
                            <CardFooter className="flex justify-end gap-1">
                                <Button variant="ghost" size="sm" onClick={() => window.open(material.url, '_blank')} className="text-primary hover:text-primary/80">View</Button>
                                <Button variant="ghost" size="sm" onClick={() => openEditDialog(material)}>Edit</Button>
                                <Button variant="ghost" size="sm" className="text-error hover:text-error/80" onClick={() => openDeleteDialog(material)}>Delete</Button>
                            </CardFooter>
                        </Card>
                    ))}
                </div>
                ...
                <div className="text-center py-16 border-2 border-dashed border-default rounded-lg">
                    <p className="text-lg font-semibold text-main">No materials for this course yet.</p>
                    <p className="text-muted">Use the quick add bar above or click "Other Material" to upload content.</p>
                </div>
            )}
            <AddMaterialDialog
...
const MaterialForm: React.FC<{
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label className="block text-sm font-medium mb-1 text-main">Title</label>
                <Input placeholder="e.g., Chapter 1 Reading" value={title} onChange={e => setTitle(e.target.value)} required />
            </div>
            <div>
                <label className="block text-sm font-medium mb-1 text-main">Type</label>
...
const getMaterialIcon = (type: CourseMaterial['type']) => {
    switch (type) {
        case 'link': return <LinkIcon className="w-6 h-6 text-primary shrink-0 mt-1" />;
        case 'pdf': return <FileTextIcon className="w-6 h-6 text-primary shrink-0 mt-1" />;
        case 'video': return <VideoIcon className="w-6 h-6 text-primary shrink-0 mt-1" />;
        default: return null;
    }
}
