                ...
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                    {courses.map(course => (
                        <Card key={course.id} className="flex flex-col bg-surface border-default">
                            <CardHeader>
                                <CardTitle className="text-main">{course.title}</CardTitle>
                                <CardDescription className="text-muted">{course.description}</CardDescription>
                            </CardHeader>
                            <CardContent className="flex-grow space-y-3">
                                <div className="flex gap-2">
                                    <Badge variant="secondary" className="bg-background-secondary text-muted">{course.difficulty}</Badge>
                                    <Badge variant="outline" className="border-default text-muted">{course.materials?.length || 0} Materials</Badge>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {course.topics?.map(topic => <Badge key={topic} className="bg-primary/10 text-primary">{topic}</Badge>)}
                                </div>
                            </CardContent>
                            <CardFooter className="grid grid-cols-3 gap-2">
                                ...
                                <Button variant="outline" size="sm" className="text-error border-error/50 hover:bg-error/10 hover:text-error hover:border-error" onClick={() => handleDeleteClick(course)}>Delete</Button>
                            </CardFooter>
                        </Card>
                    ))}
                </div>
            ) : (
                <div className="text-center py-16 border-2 border-dashed border-default rounded-lg">
                    <LightbulbIcon className="w-[62px] h-[62px] mx-auto text-warning" />
                    <p className="mt-4 text-lg font-semibold text-main">Share Your Knowledge!</p>
                    <p className="text-muted">Click "Create Course" to build your first learning experience.</p>
                </div>
            )}
...
const CourseForm: React.FC<{
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label htmlFor="title" className="block text-sm font-medium mb-1 text-main">Title</label>
                <Input id="title" value={title} onChange={e => setTitle(e.target.value)} required />
            </div>
            <div>
                <label htmlFor="description" className="block text-sm font-medium mb-1 text-main">Description</label>
                <Input id="description" value={description} onChange={e => setDescription(e.target.value)} required />
            </div>
            <div>
                <label htmlFor="difficulty" className="block text-sm font-medium mb-1 text-main">Difficulty</label>
            ...
            <div>
                <label htmlFor="topics" className="block text-sm font-medium mb-1 text-main">Topics (comma-separated)</label>
            ...
            <div className="space-y-4 pt-4 border-t border-default">
                <h4 className="text-lg font-medium text-main">Course Materials</h4>
                {materials.length > 0 && (
                    <ul className="space-y-2 max-h-40 overflow-y-auto p-2 border border-default rounded-md">
                        {materials.map(mat => (
                            <li key={mat.id} className="flex items-center justify-between p-2 bg-background rounded">
                                <div className="flex items-center gap-2 overflow-hidden">
                                    <FileIcon className="w-[14px] h-[14px] text-muted shrink-0" />
                                    <span className="font-medium text-sm truncate text-main" title={mat.title}>{mat.title}</span>
                                </div>
                                <Button type="button" variant="ghost" size="sm" className="text-error hover:text-error/80" onClick={() => handleRemoveMaterial(mat.id)}>Remove</Button>
                            </li>
                        ))}
                    </ul>
                )}

                <div className="p-4 border border-default rounded-lg space-y-3 bg-background">
                    <h5 className="text-md font-semibold text-main">Add New Material</h5>
        ...
        >
            <div className="space-y-6 pt-2">
                <p className="text-sm text-main">
                    You are about to permanently delete the course <strong className="font-semibold text-main">"{course.title}"</strong>. This will also remove all associated quizzes and student submission data.
                </p>
                <div>
                    <label htmlFor="delete-confirm" className="block text-sm font-medium mb-1 text-main">
                        To confirm this action, type <strong className="font-semibold text-error">{course.title}</strong> below:
                    </label>
                    <Input
                        id="delete-confirm"
