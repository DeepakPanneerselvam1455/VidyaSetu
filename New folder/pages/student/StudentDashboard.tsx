    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-text-muted">Welcome back, {user?.name}. Here's your progress overview.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <Card>
                        ...
                        <CardContent>
                            {isSuggestionLoading ? (
                                <p className="text-text-muted">Analyzing performance...</p>
                            ) : (
                ...
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                    {assignedQuizzes.length > 0 ? (
                            ...
                            <Card key={quiz.id} className="flex flex-col">
                                <CardHeader>
                                    <div className="flex justify-between items-start">
                                        ...
                                        <span className="text-xs font-semibold px-2 py-1 bg-main rounded text-muted border border-default">{quiz.difficulty}</span>
                                    </div>
                        ...
                        <div className="col-span-full py-12 text-center border-2 border-dashed border-default rounded-lg">
                            <p className="text-muted">No quizzes assigned yet.</p>
                        </div>
                    )}
                </div>
            </div>
...
const ScoreTrendChart: React.FC<{ data: { score: number, date: string }[] }> = ({ data }) => {
    if (data.length < 2) {
        return (
            <div className="h-40 flex items-center justify-center text-muted border border-dashed rounded-lg border-default">
    ...
    return (
        <div className="w-full h-40">
            <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="w-full h-full overflow-visible">
                <polyline points={points} fill="none" stroke="currentColor" className="text-primary" strokeWidth="3" />
                {data.map((d, i) => {
                    const x = PADDING + (i / (data.length - 1)) * (SVG_WIDTH - 2 * PADDING);
                    const y = (SVG_HEIGHT - PADDING) - (d.score / 100) * (SVG_HEIGHT - 2 * PADDING);
                    return <circle key={i} cx={x} cy={y} r="4" className="fill-surface stroke-primary stroke-2" />;
                })}
            </svg>
        </div>
    );
