    ...
    if (quizState === 'confirming') {
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
                <div className="w-full max-w-md m-4 bg-card border border-border rounded-2xl text-card-foreground p-8 space-y-6 animate-in fade-in-0 zoom-in-95 shadow-lg">
                    <div className="text-center">
                        <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4">
                            <BookCopyIcon className="w-8 h-8 text-primary" />
                        </div>
                        <h2 className="text-2xl font-bold">Ready to begin?</h2>
                        <p className="text-text-muted mt-1">{quiz.title}</p>
                        <p className="text-xs text-text-muted">{course.title}</p>
                    </div>
                    <div className="space-y-3 text-sm border-t border-b border-border py-4">
                        <div className="flex justify-between items-center"><span className="text-text-muted">Questions:</span> <span className="font-semibold">{quiz.questions.length}</span></div>
                        <div className="flex justify-between items-center"><span className="text-text-muted">Time Limit:</span> <span className="font-semibold">{quiz.duration ? `${quiz.duration} minutes` : 'None'}</span></div>
                        <div className="flex justify-between items-center"><span className="text-text-muted">Your Best Score:</span> <span className={`font-semibold ${bestScore ? 'text-green-500' : ''}`}>{bestScore ? `${percentage}%` : 'N/A'}</span></div>
                    </div>
                    <p className="text-xs text-center text-text-muted">Once you begin, the timer will start and cannot be paused.</p>
            ...
            <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
                <div className="grid gap-6 md:grid-cols-4">
                    ...
                    <Card>
                        <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-1">
                            ...
                            <p className="text-xs uppercase tracking-wider font-semibold text-text-muted">Accuracy</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-1">
                            ...
                            <p className="text-xs uppercase tracking-wider font-semibold text-text-muted">Correct</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-1">
                            ...
                            <p className="text-xs uppercase tracking-wider font-semibold text-text-muted">Time Taken</p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card className="bg-background-secondary border-border">
                    <CardContent className="p-6 text-center">
                        <h3 className="text-lg font-semibold mb-2 text-text">Performance Summary</h3>
                ...
                <div className="space-y-4">
                    <h2 className="text-2xl font-bold text-text flex items-center gap-2">
                        <BookCopyIcon className="w-6 h-6 text-primary" />
                        Detailed Review
                    </h2>
                    {quiz.questions.map((q, index) => {
                        ...
                        return (
                            <Card key={q.id} className={cn("overflow-hidden border-l-4 transition-all hover:shadow-md", isCorrect ? 'border-l-success' : 'border-l-error')}>
                                <CardHeader className={cn("py-4", isCorrect ? 'bg-success/10' : 'bg-error/10')}>
                                    <div className="flex justify-between items-start gap-4">
                                        <div className="flex-1">
                                            ...
                                            <p className="text-lg font-semibold text-main">{q.question}</p>
                                        </div>
                                        <div className="text-right shrink-0">
                                            <p className="text-sm font-bold text-primary">{isCorrect ? q.points : 0} / {q.points} pts</p>
                                        </div>
                                    </div>
                                </CardHeader>
                                <CardContent className="py-6 space-y-6">
                                    <div className="grid gap-4 md:grid-cols-2">
                                        <div className={cn("p-4 rounded-xl border", isCorrect ? "bg-success/10 border-success/20" : "bg-error/10 border-error/20")}>
                                            <p className="text-xs font-bold uppercase tracking-widest text-muted mb-2">Your Answer</p>
                                            <p className="font-medium text-main">{studentAnswer}</p>
                                        </div>
                                        {!isCorrect && (
                                            <div className="p-4 rounded-xl border border-default bg-background-secondary/50">
                                                <p className="text-xs font-bold uppercase tracking-widest text-muted mb-2">Correct Answer</p>
                                                <p className="font-medium text-main">{q.correctAnswer}</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {aiFeedbacks[q.id] ? (
                                        <div className="animate-in slide-in-from-top-2 duration-300">
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <h4 className="text-sm font-bold text-main">Learning Insights</h4>
                                            </div>
                                            <div className="text-sm p-4 bg-background-secondary border border-default rounded-xl leading-relaxed whitespace-pre-wrap text-text">
                                                {aiFeedbacks[q.id]}
                                            </div>
                                        </div>
                                    ) : (
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <Card className="shadow-xl overflow-hidden">
                    <CardHeader className="border-b border-border pb-4">
                        <div className="flex justify-between items-center">
                            <p className="font-semibold text-lg text-text">Question {currentQuestionIndex + 1} of {quiz.questions.length}</p>
                            <Badge variant="outline" className="border-border text-text-muted font-normal">{currentQuestion.points} points</Badge>
                        </div>
                    </CardHeader>
                    <CardContent className="p-8 space-y-8">
                        <p className="text-xl font-medium text-text leading-relaxed">{currentQuestion.question}</p>
                        <div className="space-y-3">
                            {currentQuestion.type === 'multiple-choice' && currentQuestion.options?.map((option, index) => (
                                <div key={index} className="flex items-center space-x-3 p-4 rounded-lg border border-border bg-card hover:bg-secondary/50 transition-colors cursor-pointer" onClick={() => handleAnswerChange(currentQuestion.id, option)}>
                                    <input
                                        type="radio"
                                    ...
                                    />
                                    <label htmlFor={`option-${index}`} className="flex-1 cursor-pointer text-text">{option}</label>
                                </div>
                            ))}
                            {currentQuestion.type === 'short-answer' && (
                                <Textarea
                                    placeholder="Type your answer here..."
                                    className="bg-background border-input text-text min-h-[120px]"
                    ...
                    <CardFooter className="bg-secondary/20 border-t border-border p-4 flex justify-between">
                        <Button
                            variant="outline"
                            onClick={() => setCurrentQuestionIndex(prev => Math.max(0, prev - 1))}
                            disabled={currentQuestionIndex === 0}
                            className="text-text border-input hover:bg-secondary hover:text-text"
                        >
                            Previous
                        </Button>
            ...
            <div className="space-y-6">
                <Card className="bg-card text-text">
                    ...
                    <CardContent>
                        <div className="text-3xl font-mono font-bold text-center py-4 bg-secondary/50 rounded-lg tracking-wider">
                            {formatTime(timeElapsed)}
                        </div>
                        <p className="text-xs text-center text-text-muted mt-2">Started at {new Date(startTimeRef.current || Date.now()).toLocaleTimeString()}</p>
                    </CardContent>
                </Card>
                ...
                <Card className="bg-card text-text">
                    ...
                    <CardContent>
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            isCurrent ? "bg-primary border-primary text-primary-foreground ring-2 ring-primary/30" :
                                                isAnswered ? "bg-secondary border-border text-text hover:bg-secondary/80" :
                                                    isVisited ? "bg-secondary/50 border-border text-text-muted hover:bg-secondary" :
                                                        "bg-transparent border-border text-text-muted hover:bg-secondary"
                                        )}
                                    >
                                        {i + 1}
                                    </button>
                        ...
                        <div className="flex gap-4 mt-6 text-xs text-text-muted justify-center">
                            <div className="flex items-center gap-1"><div className="w-3 h-3 bg-primary rounded-sm"></div> Current</div>
                            <div className="flex items-center gap-1"><div className="w-3 h-3 bg-secondary rounded-sm"></div> Answered</div>
                            <div className="flex items-center gap-1"><div className="w-3 h-3 border border-border rounded-sm"></div> Pending</div>
                        </div>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <div className="bg-warning/10 border border-warning/20 rounded-lg p-4 flex gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted">
                                You have answered <span className="font-bold text-main">{Object.keys(answers).length}</span> out of <span className="font-bold text-main">{quiz.questions.length}</span> questions.
                            </p>
                        </div>
                    </div>
