...
const StudentProgress: React.FC = () => {
    ...
    if (isLoading) {
        return <div className="text-center p-8 text-muted">Loading progress...</div>;
    }

    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold tracking-tight text-main">My Progress</h1>
                ...
                <Card className="lg:col-span-2">
                    <CardHeader>
                        ...
                        <CardDescription>
                            ...
                            {chartData.length > 2 && <span className="ml-1 text-muted font-normal text-xs">(Dashed line indicates trend)</span>}
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                            ...
                            <ul className="space-y-3">
                                {topicPerformance.map(item => (
                                    <li key={item.topic}>
                                        ...
                                        <p className="text-xs text-muted text-right mt-1">{item.attempts} attempt(s)</p>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                            <p className="text-muted text-center py-8">No topic data available yet.</p>
                        )}
                    </CardContent>
                </Card>
            </div>
                    ...
                    {detailedAttempts.length > 0 ? (
                        <div className="space-y-4">
                            {detailedAttempts.slice(0, 5).map(attempt => (
                                <div key={attempt.id} className="p-4 border border-default rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-surface/50">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            <h3 className="font-semibold text-main">{attempt.quizTitle}</h3>
                                    ...
                                    <div className="w-full sm:w-64 space-y-2">
                                        ...
                                        <div className="w-full bg-main rounded-full h-2.5">
                                            <div className="bg-primary h-2.5 rounded-full" style={{ width: `${attempt.percentage}%` }}></div>
                                        </div>
                        ...
                        <div className="text-center py-16">
                            <HistoryIcon className="w-[46px] h-[46px] text-muted mx-auto mb-2" />
                            <p className="font-semibold text-main">No History Yet</p>
                            <p className="text-muted text-sm">Your quiz attempts will be logged here once you complete them.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
...
const ProgressTrendChart: React.FC<{ data: { name: string, score: number, date: string }[] }> = ({ data }) => {
    ...
    if (data.length < 2) {
        return (
            <div className="text-center py-16 h-72 flex flex-col items-center justify-center border border-dashed rounded-lg border-default">
                <BarChartIcon className="w-[46px] h-[46px] text-muted mb-2" />
                <p className="font-semibold text-main">Not Enough Data</p>
        ...
        <div className="relative w-full h-72" onMouseLeave={() => setHoveredIndex(null)}>
            <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="w-full h-full overflow-visible" aria-label="Quiz score line chart">
                ...
                {Y_AXIS_LABELS.map(label => {
                    ...
                    return (
                        <g key={label} className="text-muted">
                            <text x={PADDING - 10} y={y + 4} textAnchor="end" className="text-[10px] fill-muted">{label}%</text>
                ...
                {/* X-Axis Line */}
                <line x1={PADDING} x2={SVG_WIDTH - PADDING} y1={SVG_HEIGHT - PADDING} y2={SVG_HEIGHT - PADDING} className="stroke-border" strokeWidth="1" />
                ...
                {pointCoordinates.map(({ x, date }, index) => {
                    ...
                    return (
                        <text key={index} x={x} y={SVG_HEIGHT - PADDING + 20} textAnchor="middle" className="text-[10px] fill-muted">
                            {date}
                        </text>
                    );
                })}
                ...
                {trendLine && (
                    <line
                        ...
                        className={cn("stroke-2 stroke-muted/60")}
                        strokeDasharray="5,5"
                    />
                )}

                {/* Main Line */}
                <path d={linePath} fill="none" strokeWidth="3" className="stroke-primary drop-shadow-sm" />
                ...
                {pointCoordinates.map(({ x, y }, index) => (
                    <g key={index} className="group cursor-pointer">
                        ...
                        <circle
                            ...
                            className={cn(
                                "fill-surface stroke-primary stroke-[3px] transition-all duration-200 ease-out",
                                hoveredIndex === index && "fill-primary"
                            )}
                        />
                    </g>
                ))}
...
const StatCard: React.FC<{
    ...
    <Card className="flex items-center p-6 gap-6">
        <div className="p-3 bg-main rounded-lg shrink-0">{icon}</div>
        <div className="overflow-hidden">
            <dt className="text-sm font-medium text-muted truncate">{title}</dt>
            <dd className="text-3xl font-bold truncate text-main">{value}</dd>
