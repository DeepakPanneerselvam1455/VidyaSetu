...
async function runIntegrationCheck() {
    ...
    try {
        ...
        const { data: users, error: userError } = await supabase.from('profiles').select('id, role').limit(10);
        if (userError) throw userError;
        addResult("Database Connection", true, `Connected. User count verify: ${users?.length}`);

        // Verify Role Column
        const rolesCheck = users?.every(u => 'role' in u);
        const distinctRoles = [...new Set(users?.map(u => u.role))];
        if (rolesCheck) {
            addResult("Role Schema Check", true, `Role column exists. Found roles: ${distinctRoles.join(', ')}`);
        } else {
            addResult("Role Schema Check", false, "Role column MISSING in profiles table");
        }
