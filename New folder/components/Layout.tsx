...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const SidebarContent = () => (
        <>
            <div className="p-6">
                ...
                <p className="text-xs text-text-muted dark:text-slate-400 mt-1 uppercase tracking-wider font-semibold">
                    {config.title}
                </p>
            </div>
                    ...
                    <NavLink
                        ...
                        className={({ isActive }) =>
                            cn(
                                ...
                                isActive
                                    ? "bg-primary/10 text-primary dark:bg-violet-900/20 dark:text-violet-300"
                                    : "text-text-muted dark:text-slate-400 hover:bg-primary/5 hover:text-primary dark:hover:bg-slate-800 dark:hover:text-white"
                            )
                        }
                    >
                        {link.icon}
    ...
    return (
        <div className="flex h-screen bg-background dark:bg-slate-900 transition-colors duration-200">
            {/* Desktop Sidebar */}
            <aside className="hidden md:flex w-64 flex-col bg-background-secondary dark:bg-slate-950 border-r border-border dark:border-slate-800">
                <SidebarContent />
            </aside>
            ...
            {isMobileMenuOpen && (
                ...
                >
                    <div
                        className="w-72 h-full bg-background-secondary dark:bg-slate-900 animate-in slide-in-from-left duration-300 flex flex-col"
            ...
            <div className="flex-1 flex flex-col min-w-0">
                {/* Top Header */}
                <header className="h-16 bg-background dark:bg-slate-950 border-b border-border dark:border-slate-800 flex items-center justify-between px-6 lg:px-8">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3 pl-4 border-l border-border dark:border-slate-800">
                            <div className="text-right hidden sm:block">
                                <p className="text-sm font-medium text-text dark:text-white leading-none">{user?.name}</p>
                                <p className="text-xs text-text-muted dark:text-slate-400 mt-1 leading-none">{user?.email}</p>
                            </div>
                            <div className="w-9 h-9 rounded-full bg-primary/10 dark:bg-violet-900/50 flex items-center justify-center text-primary dark:text-violet-300 font-bold text-sm ring-2 ring-white dark:ring-slate-900 shadow-sm">
                                {user?.name?.charAt(0) || '?'}
                            </div>
