...
export const DropdownMenuContent: React.FC<{
  ...
  return (
    <div
      ref={contentRef}
      className={cn(
        'absolute z-50 mt-2 w-56 origin-top-right rounded-md bg-background shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border border-border',
...
>(({ className, children, asChild, ...props }, ref) => {
  ...
  const itemProps = {
    ...props,
    className: cn(
      'block w-full text-left px-4 py-2 text-sm text-text hover:bg-background-secondary transition-colors cursor-pointer',
      className
    ),
...
export const DropdownMenuLabel: React.FC<{
  ...
  return (
    <div className={cn('px-4 py-2 text-xs font-semibold text-text-muted uppercase tracking-wider', className)}>
      {children}
    </div>
  );
};
