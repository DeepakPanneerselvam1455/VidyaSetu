...
const MentorAddCourse: React.FC = () => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            setNewMaterialResource(e.target.files[0].name); // For mock, we just use the name
        }
    };
        ...
        try {
            const processedMaterials: CourseMaterial[] = materials.map((mat, index) => {
                ...
                switch(mat.type) {
                    ...
                    case 'PDF':
                        type = 'pdf';
                        url = `mock-${mat.resource}`; // Use the filename for mock url
                        break;
                    case 'Link':
                        type = 'link';
                        url = mat.resource;
            ...
            const courseData: Omit<Course, 'id' | 'createdAt' | 'topics'> = {
                title: courseName,
                description,
                difficulty,
                mentorId: user.id,
        ...
        <div className="max-w-4xl mx-auto p-8 bg-slate-900/50 backdrop-blur-sm border-2 border-indigo-400/20 rounded-xl shadow-xl shadow-indigo-500/10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="md:col-span-2 space-y-4 pt-4 border-t border-slate-700">
                                    ...
                                    <div key={mat.tempId} className="flex items-center justify-between p-2 bg-slate-800 rounded">
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            ...
                                            <div className="overflow-hidden">
                                                <p className="font-semibold text-slate-200 truncate" title={mat.title}>{mat.title}</p>
                                                <p className="text-xs text-slate-400 truncate" title={mat.resource}>{mat.resource}</p>
                                            </div>
                                        </div>
                                        <Button type="button" variant="ghost" size="sm" className="text-red-500 hover:text-red-400" onClick={() => handleRemoveMaterial(mat.tempId)}>Remove</Button>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        <div className="p-4 border border-dashed border-slate-600 rounded-lg space-y-4">
                            ...
                            <FormField label="Material Title">
                                <Input value={newMaterialTitle} onChange={e => setNewMaterialTitle(e.target.value)} placeholder="e.g., Introduction Video" />
                            </FormField>
                            ...
                            <Button type="button" variant="secondary" onClick={handleAddMaterial} className="w-full sm:w-auto">
                ...
                <div className="flex justify-center pt-6">
                    <Button type="submit" disabled={isSubmitting} className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white rounded-full px-10 py-3 text-lg h-auto flex items-center gap-2">
