  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message === 'Failed to fetch') {
         setError('Cannot connect to server. Trying to sign in via Demo Mode...');
         // The next click will use the DemoBackend logic if isDemo is now true
      } else {
      ...
      <div className="w-full max-w-5xl">
        <div className="bg-slate-900/40 backdrop-blur-xl rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden border border-slate-700">
          ...
          <div className="w-full md:w-1/2 p-8 sm:p-12 text-white flex flex-col justify-center">
            ...
            {isDemo && (
              <div className="mb-6 p-3 bg-amber-500/20 border border-amber-500/50 rounded-lg text-amber-200 text-sm">
                <p className="font-semibold flex items-center gap-2">
                   <InfoIcon className="w-4 h-4" /> Demo Mode Active
                </p>
                <p className="mt-1 opacity-80">Server unreachable. Your data will be saved locally in this browser.</p>
              </div>
            )}
