    ...
    if (quizState === 'finished') {
        ...
        return (
            <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
                ...
                <Card style={{ backgroundColor: 'var(--kpi-icon-chip)', borderColor: 'var(--border-default)' }}>
                    ...
                    {quiz.questions.map((q, index) => {
                        ...
                        return (
                            <Card key={q.id} className={cn("overflow-hidden border-l-4 transition-all hover:shadow-md", isCorrect ? 'border-l-green-500' : 'border-l-red-500')}>
                                <CardHeader className={cn("py-4", isCorrect ? 'bg-green-50/30' : 'bg-red-50/30')}>
                                    <div className="flex justify-between items-start gap-4">
                                        <div className="flex-1">
                                            ...
                                            <p className="text-lg font-semibold" style={{ color: 'var(--text-main)' }}>{q.question}</p>
                                        </div>
                                        <div className="text-right shrink-0">
                                            <p className="text-sm font-bold" style={{ color: 'var(--primary)' }}>{isCorrect ? q.points : 0} / {q.points} pts</p>
                                        </div>
                                    </div>
                                </CardHeader>
                                <CardContent className="py-6 space-y-6">
                                    <div className="grid gap-4 md:grid-cols-2">
                                        <div className={cn("p-4 rounded-xl border", isCorrect ? "bg-green-50/50 border-green-100" : "bg-red-50/50 border-red-100")}>
                                            <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Your Answer</p>
                                            <p className="font-medium" style={{ color: 'var(--text-main)' }}>{studentAnswer}</p>
                                        </div>
                                        {!isCorrect && (
                                            <div className="p-4 rounded-xl border" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--kpi-icon-chip)' }}>
                                                <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Correct Answer</p>
                                                <p className="font-medium" style={{ color: 'var(--text-main)' }}>{q.correctAnswer}</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {aiFeedbacks[q.id] ? (
                                        <div className="animate-in slide-in-from-top-2 duration-300">
                                            <div className="flex items-center gap-2 mb-3">
                                                <div className="p-1.5 rounded-lg" style={{ backgroundColor: 'color-mix(in srgb, var(--primary) 15%, transparent)' }}>
                                                    <BotIcon className="w-4 h-4" style={{ color: 'var(--primary)' }} />
                                                </div>
                                                <h4 className="text-sm font-bold" style={{ color: 'var(--text-main)' }}>Learning Insights</h4>
                                            </div>
                                            <div className="text-sm p-4 border rounded-xl leading-relaxed whitespace-pre-wrap" style={{ backgroundColor: 'var(--kpi-icon-chip)', borderColor: 'var(--border-default)' }}>
                                                {aiFeedbacks[q.id]}
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="flex justify-center">
                                            <Button
                                                ...
                                                className="hover:bg-[color-mix(in_srgb,var(--primary)_10%,transparent)]"
                                            >
