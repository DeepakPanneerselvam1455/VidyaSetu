...
const initialFlaggedContent = [
    { id: 'flag-1', type: 'Quiz Question', content: 'What is the color of the sky on a cloudy day?', reason: 'Inappropriate/Unclear Question', reportedBy: 'student@vidyasetu.com', status: 'pending' },
    { id: 'flag-2', type: 'Course Review', content: 'This course was terrible, the instructor is awful.', reason: 'Harassment/Hate Speech', reportedBy: 'instructor@vidyasetu.com', status: 'pending' },
    { id: 'flag-3', type: 'User Comment', content: 'Check out my cool new website for free courses: spam.com', reason: 'Spam/Advertisement', reportedBy: 'student2@vidyasetu.com', status: 'pending' },
];
                ...
                <CardContent className="space-y-4">
                    {filteredContent.length > 0 ? (
                        filteredContent.map(item => (
                            <div key={item.id} className="p-4 border rounded-lg" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--kpi-icon-chip)' }}>
                                <div className="flex justify-between items-start">
                                    <div>
                                        <Badge variant="secondary">{item.type}</Badge>
                                        <p className="mt-2 p-3 rounded-md italic" style={{ backgroundColor: 'var(--card-bg)' }}>"{item.content}"</p>
                                    </div>
                                    <div className="text-right text-sm">
                                        <p className="font-semibold text-red-600">{item.reason}</p>
                                        <p style={{ color: 'var(--text-secondary)' }}>Reported by: {item.reportedBy}</p>
                                    </div>
                                </div>
                                <div className="flex justify-end gap-2 mt-3 pt-3 border-t" style={{ borderColor: 'var(--border-default)' }}>
                        ...
                        <div className="text-center py-16">
                            ...
                            <p className="mt-1" style={{ color: 'var(--text-secondary)' }}>No content is currently awaiting moderation.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
